...
class chaosaiart_CheckpointPrompt:
    ...
    def node(self, Checkpoint,Positiv,Negativ,Add_Prompt_txt,add_Positiv_txt = "",add_Negativ_txt=""):
        ...
        mode_bool = True if Add_Prompt_txt == "After" else False
        sPositiv = chaosaiart_higher.add_Prompt_txt_byMode(Positiv,add_Positiv_txt,mode_bool)
        sNegativ = chaosaiart_higher.add_Prompt_txt_byMode(Negativ,add_Negativ_txt,mode_bool)  
...
class chaosaiart_EmptyLatentImage:
    ...
    CATEGORY = "Chaosaiart/image_video"
...
class chaosaiart_CheckpointPrompt2:
    ...
    def node(self, Checkpoint,positiv_txt,negativ_txt):
...
class chaosaiart_CheckpointPrompt_Frame:
    ...
    def node(self, Start_Frame,Checkpoint,Positiv,Negativ): 
        return ([Start_Frame,Checkpoint,Positiv,Negativ],)
...
class chaosaiart_KSampler3: 
    ...
    def INPUT_TYPES(s):
        return {    
            "required":{
                    ...
                    "Image_width": ("INT", {"default": 512, "min": 16, "max": MAX_RESOLUTION, "step": 8}),
                    "Image_height": ("INT", {"default": 512, "min": 16, "max": MAX_RESOLUTION, "step": 8}),
                    "Image_batch_size": ("INT", {"default": 1, "min": 1, "max": 4096})
                },
                "optional":{ 
                    "denoise_Override": ("FLOAT",{"forceInput": True}),  
    ...
    def node(self, model, vae, seed, steps, cfg, sampler_name, scheduler, positive, negative, denoise,Image_width,Image_height,Image_batch_size,latent_Override=None,latent_by_Image_Override=None,denoise_Override=None):
...
class chaosaiart_TextCLIPEncode:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "positiv_txt": ("STRING", {"multiline": True, "forceInput": True}),
                "negitiv_txt": ("STRING", {"multiline": True, "forceInput": True}),
                "clip": ("CLIP", )
            }}
    ...
    CATEGORY = "Chaosaiart/prompt"
    def node(self, clip, positiv_txt,negitiv_txt):
        return (chaosaiart_higher.textClipEncode(clip,positiv_txt),chaosaiart_higher.textClipEncode(clip,negitiv_txt), )
...
class chaosaiart_Prompt_Frame: 
    ...
    def node(self,start_frame,positiv="",negativ="",add_positiv=None,add_negativ=None):
       
        positivOUT = add_positiv if add_positiv else ""
        negativOUT = add_negativ if add_negativ else ""

        if not positiv == "" and not positivOUT=="" : 
            positivOUT += ","    

        if not negativ == "" and not negativOUT=="" : 
            negativOUT += ","    
       
        positivOUT += positiv
        negativOUT += negativ
         
        return([start_frame,[positivOUT,negativOUT]],)
...
class chaosaiart_Prompt: 
    ...
    RETURN_TYPES = ("STRING","STRING","MAIN_PROMPT",)
    RETURN_NAMES = ("POSITIV_TXT","NEGATIV_TXT","MAIN_PROMPT")

    FUNCTION = "node"

    CATEGORY = "Chaosaiart/prompt"
    
    def node(self,positiv="",negativ="",add_positiv=None,add_negativ=None):
        ...
        positivOUT += positiv
        negativOUT += negativ
        return(positivOUT,negativOUT,[positivOUT,negativOUT])
...
class chaosaiart_Prompt_mixer_byFrame: 
    ...
    RETURN_TYPES = ("STRING","STRING","STRING","FRAME_PROMPT",)
    RETURN_NAMES = ("Info","POSITIV_TXT","NEGAITV_TXT","FRAME_PROMPT_X",)
    ...
    def node(self,activ_frame=0,
        ...
        if main_prompt:
            positiv += str(main_prompt[0]) + ","
            negativ += str(main_prompt[1]) + ","

        frame_key = 0
        Frame_sorted_inputs = sorted(filter(None, Frame_array), key=lambda x: x[0], reverse=True)

        print(Frame_sorted_inputs)

        for array in Frame_sorted_inputs:
            if isinstance(array, list): 
                if activ_frame >= int(array[0]):

                    positiv  += str(array[1][0])
                    negativ  += str(array[1][1])
                    frame_key = int(array[0])
                    break
                     
        info = f"Frame: {activ_frame},\nPositiv: {positiv},\nNegativ: {negativ}"
        return(info,positiv,negativ,[frame_key,[positiv,negativ]])
...
def bilder_zu_video():
    # Eingabe des Bildordners
    bilder_ordner = easygui.diropenbox(msg='Bitte wählen Sie den Bildordner aus')

    if not os.path.isdir(bilder_ordner):
        easygui.msgbox('Ungültiger Bildordner ausgewählt')
        return
    ...
    # Eingabe des Ausgabedateinamens
    ausgabedatei = easygui.enterbox(msg='Geben Sie den Ausgabedateinamen ein', default='ausgabe_video.mp4')

    # Eingabe der FPS
    fps = easygui.enterbox(msg='Geben Sie die FPS ein', default='10')
    fps = int(fps)
    ...
    # Prüfe, ob im Bildordner Bilddateien vorhanden sind
    if not bilddateien:
        easygui.msgbox('Keine Bilddateien im Bildordner gefunden')
        return
    ...
    for datei in bilddateien:
        ...
        # Zeige das aktuelle Bild im Fenster an (optional)
        cv2.imshow('Bild zum Video', bild)
        cv2.waitKey(1)  # Warte 1 Millisekunde zwischen den Bildern

    # Schließe das Video und das Fenster
    video.release()

    
    print(f'Video Erstellt:')
    print(f'ordner: {output_ordner}{ausgabedatei}')

    
    text1 = f'Video Erstellt:'
    text2 = f'ordner: {output_ordner}{ausgabedatei}'

    nachricht = f'{text1}\n\n{text2}'
    easygui.msgbox(nachricht)
...
NODE_CLASS_MAPPINGS = {
    ...
    #"chaosaiart_imgHolder": chaosaiart_ImageHolder,
    #"chaosaiart_ImageHolder_loop": chaosaiart_ImageHolder_loop,
    ...
    #"chaosaiart_cn_math":chaosaiart_controlnet_Math,
    ...
    #"chaosaiart_Batch_Loader_Number_Override": chaosaiart_Batch_Loader_Number_Override,
    ...
    #"chaosaiart_KSampler":chaosaiart_KSampler, 
    ...
    #"chaosaiart_Any_Display": chaosaiart_DisplayAny,
    #"chaosaiart_test": chaosaiart_test,
    #"chaosaiart_test2": chaosaiart_test2, 
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    #"chaosaiart_imgHolder": "Image Holder",
    #"chaosaiart_ImageHolder_loop": "Image Holder Loop",
    ...
    "chaosaiart_TextCLIPEncode_simple": "🔶 Simple Text Prompt Clip Encode",
    #"chaosaiart_cn_math": "controlnet Math start stop step",
    ...
    #"chaosaiart_Batch_Loader_Number_Override": "Batch Loader Number_Override",
    ...
    "chaosaiart_CheckpointPrompt": "🔶 Load Checkpoint +Prompt",
    "chaosaiart_CheckpointPrompt2": "🔶 Load Checkpoint +positiv_txt negativ_txt",
    "chaosaiart_CheckpointLoader":"🔶 Load Checkpoint by Frame +positiv_txt negativ_txt",
    "chaosaiart_CheckpointPrompt_Frame": "🔶 Load Checkpoint +Prompt +Frame CKPT_PROMPT",
    "chaosaiart_CheckpointPrompt_FrameMixer": "🔶 Checkpoint mixer by Frame",
    "chaosaiart_restarter_advanced": "🔶 Restart & Activ - Advanced",
    #"chaosaiart_Any_Display": "Info Any Display",
    "chaosaiart_Show_Info": "🔶 Info Display",
    #"chaosaiart_Show_Any": "Any Display"
